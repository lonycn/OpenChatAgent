...
describe("SessionManager", () => {
  ...
  describe("constructor", () => {
    ...
    it("should throw an error if the redis client is invalid (e.g., missing methods)", () => {
      ...
      expect(() => new SessionManager(invalidClient)).toThrow(
  ...
  describe("Key Generation (private methods)", () => {
    ...
    it("_getHistoryKey should return correct format", () => {
      expect(sessionManager._getHistoryKey("s123")).toBe(
        "session:s123:history"
      );
    });
  });
  ...
  describe("createSession", () => {
    ...
    it("should create a session successfully", async () => {
      ...
      expect(mockPipeline.hset).toHaveBeenCalledWith(
        `session:${mockSessionId}:meta`,
        expectedMeta
      );
      expect(mockPipeline.set).toHaveBeenCalledWith(
        `session:${mockSessionId}:agent`,
        "ai"
      );
    ...
    it("should throw an error if userId is missing", async () => {
      await expect(sessionManager.createSession(null)).rejects.toThrow(
    ...
    it("should handle Redis pipeline execution failure", async () => {
      ...
      await expect(sessionManager.createSession(mockUserId)).rejects.toThrow(
        "Failed to create session: Redis pipeline failed"
      );
    });
    ...
    it("should handle individual command failure within pipeline (simulated by exec throwing)", async () => {
      ...
      mockPipeline.exec.mockImplementation(async () => {
        throw new Error(
          "Simulated pipeline command failure (e.g. HMSET failed)"
        );
      });
      await expect(sessionManager.createSession(mockUserId)).rejects.toThrow(
        "Failed to create session: Simulated pipeline command failure (e.g. HMSET failed)"
      );
  ...
  describe("getSessionAgent", () => {
    ...
    it("should throw an error if sessionId is missing", async () => {
      await expect(sessionManager.getSessionAgent(null)).rejects.toThrow(
    ...
    it("should handle Redis GET failure", async () => {
      ...
      await expect(sessionManager.getSessionAgent("s123")).rejects.toThrow(
        "Failed to get session agent: Redis GET failed"
      );
    });
  });
  ...
  describe("switchAgent", () => {
    ...
    it("should throw if sessionId or newAgent is missing", async () => {
      await expect(sessionManager.switchAgent(null, newAgent)).rejects.toThrow(
        "sessionId and newAgent are required."
      );
      await expect(
        sessionManager.switchAgent(mockSessionId, null)
      ).rejects.toThrow("sessionId and newAgent are required.");
    });

    it("should handle Redis pipeline failure", async () => {
      mockPipeline.exec.mockRejectedValue(new Error("Redis pipeline failed"));
      await expect(
        sessionManager.switchAgent(mockSessionId, newAgent)
      ).rejects.toThrow("Failed to switch agent: Redis pipeline failed");
    });
  });
  ...
  describe("getSession", () => {
    ...
    it("should throw if sessionId is missing", async () => {
      await expect(sessionManager.getSession(null)).rejects.toThrow(
        "sessionId is required."
      );
    });
    ...
    it("should handle Redis HGETALL failure", async () => {
      ...
      await expect(sessionManager.getSession("s123")).rejects.toThrow(
        "Failed to get session: Redis HGETALL failed"
      );
    });
  });
  ...
  describe("isSessionActive", () => {
    ...
    it("should throw if sessionId is missing", async () => {
      await expect(sessionManager.isSessionActive(null)).rejects.toThrow(
        "sessionId is required."
      );
    });
    ...
    it("should handle Redis EXISTS failure", async () => {
      ...
      await expect(sessionManager.isSessionActive("s123")).rejects.toThrow(
        "Failed to check session activity: Redis EXISTS failed"
      );
    });
  });
  ...
  describe("extendSession", () => {
    ...
    it("should throw if sessionId is missing", async () => {
      await expect(sessionManager.extendSession(null)).rejects.toThrow(
        "sessionId is required."
      );
    });
    ...
    it("should handle Redis HSET failure", async () => {
      ...
      await expect(sessionManager.extendSession(mockSessionId)).rejects.toThrow(
        "Failed to extend session: Redis HSET failed"
      );
    });
  });
  ...
  describe("addMessage", () => {
    ...
    beforeEach(() => {
      ...
      jest
        .spyOn(sessionManager, "extendSession")
        .mockResolvedValue({ success: true });
      mockRedis.rpush.mockResolvedValue(1);
      mockRedis.llen.mockResolvedValue(5); // Mock message count
    ...
    it("should add a message successfully", async () => {
      ...
      expect(sessionManager.isSessionActive).toHaveBeenCalledWith(
        mockSessionId
      );
    ...
    it("should throw an error if sessionId is missing", async () => {
      await expect(
        sessionManager.addMessage(null, validMessage)
      ).rejects.toThrow("sessionId is required to add a message.");
    });

    it("should throw an error for invalid message structure", async () => {
      const invalidMsg = { from: "user", text: "Hi" };
      await expect(
        sessionManager.addMessage(mockSessionId, invalidMsg)
      ).rejects.toThrow(
    ...
    it("should throw an error if session is not active", async () => {
      ...
      await expect(
        sessionManager.addMessage(mockSessionId, validMessage)
      ).rejects.toThrow(
        `Session ${mockSessionId} does not exist or has expired.`
      );
    });
    ...
    it("should handle Redis RPUSH failure", async () => {
      ...
      await expect(
        sessionManager.addMessage(mockSessionId, validMessage)
      ).rejects.toThrow("Failed to add message: Redis RPUSH failed");
    });

    it("should handle extendSession failure", async () => {
      sessionManager.extendSession.mockRejectedValue(
        new Error("Extend session failed")
      );
      await expect(
        sessionManager.addMessage(mockSessionId, validMessage)
      ).rejects.toThrow("Failed to add message: Extend session failed");
    });
  });
  ...
  describe("getHistory", () => {
    ...
    it("should handle unparseable JSON gracefully", async () => {
      ...
      expect(history).toEqual([
        msg1,
        { error: "Failed to parse message data", raw: "invalid-json" },
        msg2,
      ]);
    ...
    it("should throw if sessionId is missing", async () => {
      await expect(sessionManager.getHistory(null)).rejects.toThrow(
        "sessionId is required to get history."
      );
    });

    it("should handle Redis LRANGE failure", async () => {
      mockRedis.lrange.mockRejectedValue(new Error("Redis LRANGE failed"));
      await expect(sessionManager.getHistory(mockSessionId)).rejects.toThrow(
        "Failed to get history: Redis LRANGE failed"
      );
    });
  });
  ...
  describe("setSessionTTL", () => {
    ...
    it("should throw if sessionId is missing", async () => {
      await expect(sessionManager.setSessionTTL(null, ttl)).rejects.toThrow(
        "sessionId is required."
      );
    });
    ...
    it("should throw if ttlInSeconds is invalid", async () => {
      await expect(
        sessionManager.setSessionTTL(mockSessionId, 0)
      ).rejects.toThrow("ttlInSeconds must be a positive number.");
      await expect(
        sessionManager.setSessionTTL(mockSessionId, -100)
      ).rejects.toThrow("ttlInSeconds must be a positive number.");
      await expect(
        sessionManager.setSessionTTL(mockSessionId, "abc")
      ).rejects.toThrow("ttlInSeconds must be a positive number.");
    });
    ...
    it("should throw if session is not active", async () => {
      sessionManager.isSessionActive.mockResolvedValue(false);
      await expect(
        sessionManager.setSessionTTL(mockSessionId, ttl)
      ).rejects.toThrow(
        `Session ${mockSessionId} not found or has already expired.`
      );
    });
    ...
    it("should handle Redis pipeline failure", async () => {
      ...
      await expect(
        sessionManager.setSessionTTL(mockSessionId, ttl)
      ).rejects.toThrow("Failed to set session TTL: Redis pipeline failed");
    });
  });
  ...
  describe("getSessionTTL", () => {
    ...
    it("should throw if sessionId is missing", async () => {
      await expect(sessionManager.getSessionTTL(null)).rejects.toThrow(
        "sessionId is required."
      );
    });
    ...
    it("should handle Redis TTL failure", async () => {
      ...
      await expect(sessionManager.getSessionTTL(mockSessionId)).rejects.toThrow(
        "Failed to get session TTL: Redis TTL failed"
      );
    });
  });
});
