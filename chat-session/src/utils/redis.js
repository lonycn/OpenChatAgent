...
require('dotenv').config({ path: require('path').resolve(__dirname, '../../.env') }); // Load .env from chat-session root
...
function getRedisClient() {
  ...
  console.log(`Attempting to connect to Redis: ${redisHost}:${redisPort}, DB: ${redisDb}`);
  ...
  if (isNaN(connectionOptions.port)) {
    console.error('Redis: Invalid REDIS_PORT. Please check your .env file. Using default 6379 instead if possible.');
  ...
  if (isNaN(connectionOptions.db)) {
    console.error('Redis: Invalid REDIS_DB. Please check your .env file. Using default 0 instead if possible.');
  ...
  client.on('connect', () => {
    console.log(`Redis: Successfully connected to ${redisHost}:${redisPort}, DB: ${redisDb}`);
  });
